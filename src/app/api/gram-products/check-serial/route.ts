...
export async function GET(request: NextRequest) {
  ...
  if (!name || !serialPrefix) {
    return NextResponse.json({ error: "Name and serialPrefix are required" }, { status: 400 });
  }

  try {
    const normalizedPrefix = normalizeSerialCode(serialPrefix);

    // Find all gram product items with the same batch name and serial prefix
    ...
    const serials = items.map((item) => item.serialCode);
    const lastNumber = findHighestSerialNumber(serials, normalizedPrefix);
    const nextNumber = lastNumber + 1;

    return NextResponse.json({
      exists: true,
      lastSerial: items[0]?.serialCode || null,
      lastNumber,
      nextNumber,
      totalExisting: items.length,
      message: `Found ${items.length} existing product(s). Last serial: ${items[0]?.serialCode || "N/A"}. Next will start from ${nextNumber}.`,
    });
  } catch (error: any) {
